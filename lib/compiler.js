...
class Compiler
{
    ...
    compileImport(scope, sourcePath, source, objjScope)
    {
        ...
        catch (e)
        {
            ...
            throw new exceptions.CompileAbortedError("" + this.printIssue(e, sourcePath));
    ...
    */
    ...
    static findPreviousVarDeclaration(identifierNode)
    {
        ...
        try {
            ...
            console.error(statement.sourceFile);
            console.error("At line: " + statement.loc.start.line + ":");
            console.error(e);
        }
    ...
    /*
        ...
        property: Object - Keys are property attribute names, values are:
            name, getter, setter - Identifier node
            readwrite, readonly, copy - true
    */
    generateProperties(node, classDef) // -> property
    {
        ...
        for (let i = 0; i < ivars.length; i++)
        {
            ...
            let attributes = this.constructor.propertyAttributes(properties),
    ...
    static propertyAttributes(properties)
    {
        let attributes = [];

        if (properties.readonly)
            attributes.push("readonly");
        else if (properties.copy)
        ...
        if (properties.property)
            attributes.push(`property=${properties.property.name}`);

        if (properties.getter)
            attributes.push(`getter=${properties.getter.name}`);

        if (properties.setter)
            // Strip trailing ':'
            attributes.push(`setter=${properties.setter.name.replace(/(.+):$/, "$1")}`);
    ...
    static generateGetter(classDef, property, properties, node, ivarName, ivarType)
    {
        const selector = this.makePropertySelector(property, "getter", ivarName);
        ...
        const code = `
// ${ivarName} @property${properties} [getter]
// - (${ivarType})${selector}
...
→return self.${ivarName};
    ...
    generateSetter(classDef, property, properties, node, ivarName, ivarType)
    {
        let selector = this.constructor.makePropertySelector(property, "setter", ivarName);
...
→→/* ${ivarName} = [newValue copy] */ self.${ivarName} = newValue == null ? null : ${messageSend}(newValue, "copy");`;
            ...
            setterCode =
                `if (self.${ivarName} !== newValue) {
→→ self.${ivarName} = newValue;`
        }

        if(property.display) {
            setterCode += '\n\n→→if(!!class_getInstanceMethod(self.isa, sel_getUid("setNeedsDisplay:"))){';
            setterCode += '\n→→/* [self setNeedsDisplay:YES];*/ self.isa.objj_msgSend(self, "setNeedsDisplay:", YES);';
            setterCode += '\n→→}'
        }

        if(property.layout) {

            setterCode += '\n\n→→if(!!class_getInstanceMethod(self.isa, sel_getUid("setNeedsLayout:"))){';
            setterCode += '\n→→/* [self setNeedsLayout:YES];*/ self.isa.objj_msgSend(self, "setNeedsLayout:", YES);';
            setterCode += '\n→→}'
        ...
        const definition = `
// ${ivarName} @property${properties} [setter]
// - (void)${selector}(${ivarType})newValue
new objj_method(sel_getUid("${selector}"),
function(self, _cmd, newValue)
{
→${setterCode}
})`;
    ...
    static makePropertySelector(property, type, ivarName)
    {
        let selector;

        if (property[type])
            selector =property[type].name.replace(/(.+):$/, "$1"); // strip trailing ':'
        else if (property.name)
            selector = property.name.name;
    ...
    // If an ivar has a readonly @property, the class may not define
    ...
    checkForSetterConflicts(node, scope, selector)
    {
        ...
        for (const entry of ivars)
        {
            ...
            if (!ivar.property || !ivar.property.readonly)
                continue;

            // If the property specifies a custom setter name, use that
            let name = entry[0],
                setter;

            if (ivar.property.name)
                name = ivar.property.name.name;
            else if (ivar.property.setter)
                setter = ivar.property.setter.name;
