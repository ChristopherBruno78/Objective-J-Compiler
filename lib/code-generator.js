...
*/
function addIvars(node, compiler, classDef) // -> hasProperties
{
    const
        buffer = compiler.jsBuffer,
...
function importDeclaration(node, scope) {
	const compiler = scope.compiler,
		  buffer = compiler.jsBuffer; //pass through
	buffer.concat("//");
...
{
    const filename = node.objj.filename,
        buffer = scope.compiler.jsBuffer;

    if (filename) {
        let filePath = filename.trim();
        buffer.concat("//import \"" + filePath + "\"\n");
     }
...
`;
	...
	let superclassName = classDef.category ? classDef.category : classDef.superclassNode;
    ...
    if (objj.ivars && objj.ivars.length > 0)
        haveProperties = addIvars(node, compiler, classDef);
    ...
    const haveMethods = compiler.instanceMethods.length || haveProperties;

    if (haveMethods)
        buffer.concat(indentString("\n\n// Instance methods\nclass_addMethods($the_class,\n["));

    if (haveProperties)
        compiler.generateProperties(node, classDef);
...
const generators =
    {
        ...
        JSXNamespacedName : jsx.JSXNamespacedName,
